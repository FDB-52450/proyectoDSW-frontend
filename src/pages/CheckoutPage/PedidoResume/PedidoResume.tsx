...
export function PedidoResume({cart}: {cart: PedidoProd[]}) {
    ...
    const textSize = isSmallMobile ? 'xs' : 'sm'
    const width = isSmallMobile ? '60%' : '80%'
        ...
        <>
            ...
            <Stack>
                ...
                {cart.map((cartProd: PedidoProd) => (
                    <Group justify="space-between">
                        <Group gap={5} w={width}>
                            <Text size={textSize} c='blue'>[x{cartProd.cantidad}]</Text>
                            <Text size={textSize} truncate="end" style={{whiteSpace: 'normal', overflowWrap: 'break-word', maxWidth: '90%'}}>{cartProd.producto.nombre}</Text>
                        </Group>
                        <Text size={textSize}>${(cartProd.producto.precioFinal * cartProd.cantidad).toLocaleString('es-AR')}</Text>
                    </Group>
                ))}
                <Divider/>
                <Group justify="space-between">
