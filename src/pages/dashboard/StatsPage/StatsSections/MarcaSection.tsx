...
import type { MarcaStatsDTO } from '../../../../entities/stats.ts'

function TableSoldRows({data, tipo}: {data: MarcaStatsDTO, tipo: 'most' | 'least'}) {
...
export function MarcaSection({data}: {data: MarcaStatsDTO}) { 
    const highestMarcaName = data.ventas[0]?.nombre ?? '-'
    const lowestMarcaName = data.ventas[data.ventas.length - 1]?.nombre ?? '-'
        ...
        <>
            <Grid.Col span={spanSize}>
                <Stack align='center' gap={5} className={styles.statsContainer} h='380px'>
                    <Text fw={500} size={smallTextSize}>Marca mas vendida</Text>        
                    {data.ventas.length > 0 ? 
                    <>
                        <Text fw={500} size={bigTextSize} mb={25}>{highestMarcaName}</Text>
                        <Table>
            ...
            <Grid.Col span={spanSize}>
                <Stack align='center' gap={5} className={styles.statsContainer} h='380px'>
                    <Text fw={500} size={smallTextSize}>Marca menos vendida</Text>
                    {data.ventas.length > 0 ? 
                    <>
                        <Text fw={500} size={bigTextSize} mb={25}>{lowestMarcaName}</Text>
                        <Table>
                            <Table.Thead>{tableHeaders}</Table.Thead>
                            <Table.Tbody>
